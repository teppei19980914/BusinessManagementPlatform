...
const mockedWorkload = vi.mocked(getAssigneeWorkload);
...
describe('認証・認可', () => {
  ...
  it("checkProjectPermission に 'analytics:read' を渡す", async () => {
    ...
    mockedWorkload.mockResolvedValueOnce(SAMPLE);
...
describe('正常系', () => {
  ...
  it('service 結果を { data } で返却', async () => {
    ...
    expect(body.data.assignees[0].totalPlanned).toBe(10);
    expect(body.data.assignees[0].paceRatio).toBe(1.2);
  });

  it('[テナント分離] projectId と viewerTenantId が渡る', async () => {
    mockedWorkload.mockResolvedValueOnce(SAMPLE);
    await GET(new Request('http://localhost/x') as never, mockParams('p1'));
    expect(mockedWorkload).toHaveBeenCalledWith('p1', TENANT_A);
  });
});
