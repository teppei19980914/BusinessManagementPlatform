...
/**
 * 関数の実行時間上限を 60 秒に設定。
 * 現行の Netlify Personal Sync Function 上限は 10 秒固定、Pro 以上で 26 秒まで configurable のため、
 * `maxDuration = 60` は将来的な Netlify Pro 昇格 / Background Functions 化を見越した宣言値。
 * 通常は A 案 (祖先伝播スキップ) + C 案 (一致時 update スキップ) で十分高速に完了するが、
 * WP 数が多い・全 WP が未反映のデータを 1 回で修復するケースでは既定 10 秒を超える可能性が
 * あるため、長期的に 60 秒まで許容する設計。
 */
