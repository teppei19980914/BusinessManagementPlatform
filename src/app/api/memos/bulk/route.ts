...
/**
 * 「全メモ」横断ビューからの一括 visibility 更新 (PR #162 / Phase 2)。
 *
 * Memo は visibility 値域が `private` / `public` (DB schema) なので
 * 共通 schema は entity 別の enum に分かれている (cross-list-bulk-visibility.ts 参照)。
 *
...
export async function PATCH(req: NextRequest) {
  ...
  const result = await bulkUpdateMemosVisibilityFromCrossList(
    parsed.data.ids,
