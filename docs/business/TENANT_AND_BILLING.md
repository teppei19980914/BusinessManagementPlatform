...
**Beginner プラン**: 試験運用と小〜中規模プロジェクト向けの **無料プラン**。最大 5 席、Claude Haiku、月間 100 回までの API 呼び出し上限。上限到達時は **縮退モード**（§34.14.4 / NF-13.14、エンティティ作成・更新は継続、AI 裏方処理のみ一時停止）に切り替わる。上位プランへのアップセル誘導の入り口として機能する。

**Expert プラン**: 中〜大規模チームの本格利用向けの **席数無制限・従量課金プラン**。Claude Haiku、API 呼び出し 1 回あたり **¥5** (2026-05-15 改定: ¥10 → ¥5)。月間使用量に上限なし、使った分だけ請求される。

**Pro プラン**: PMO や経営層など「助言の質」を重視するユーザ向けの **席数無制限・従量課金プラン**。Claude Sonnet、API 呼び出し 1 回あたり **¥15** (2026-05-15 改定: ¥30 → ¥15)。Sonnet による深い説明文付きの提案を享受できる最上位プラン。
...
**NF-13.14 縮退モードの優雅な動作**: API 呼び出し上限到達時 (Beginner 月間 100 回、または Expert/Pro の月次予算上限) は、**「業務に必要な作成・更新は継続 + AI による裏方処理のみ一時停止」** する縮退モードに切り替わる。詳細仕様は §34.14.4 参照。
...
**Expert プラン** は席数に上限なし、Claude Haiku、API 呼び出し 1 回あたり **¥5** の従量課金 (2026-05-15 改定: ¥10 → ¥5)。月間使用量に上限はなく、使った分だけ請求される。中〜大規模チームで日常的に提案機能を活用するユーザを想定する。

**Pro プラン** は席数に上限なし、Claude Sonnet、API 呼び出し 1 回あたり **¥15** の従量課金 (2026-05-15 改定: ¥30 → ¥15)。Sonnet による深い説明文付きの提案を享受できる最上位プランで、PMO や経営層など「助言の質」を重視するユーザを想定する。Haiku 構成と Pro 構成の差は、検索精度の根幹ではなく説明文の文脈読解の深さに現れる。
...
**Expert プラン** は **席数無制限の従量課金プラン** で、Claude Haiku で動作する。1 回の API 呼び出しごとに **¥5** が課金され、月間使用量に上限はない (= 使った分だけ請求される)。主に中〜大規模チームで日常的に提案機能を使うユーザを想定する。

**Pro プラン** は **席数無制限の従量課金プラン** で、Claude Sonnet で動作する。1 回の API 呼び出しごとに **¥15** が課金される。Sonnet による深い説明文付きの提案を享受でき、PMO や経営層など「助言の質」を重視するユーザに向けた最上位プランである。

**2026-05-15 価格改定**: 初期 ¥10 / ¥30 から **半額の ¥5 / ¥15 に改定** (=ユーザ採用ハードル削減)。半額化後も粗利 73〜75% を維持し、ワーストケース (Anthropic 値上げ 2 倍 + Voyage 無料枠超過) でも 50%+ の粗利が確保される。詳細は [ADR-0002](../adr/0002-tenant-billing-per-api-call.md) の改定 entry を参照。
...
**確定仕様**: API 呼び出し上限（Beginner: 月 100 回 / Expert・Pro: テナント管理者設定の予算上限）に達した時点で、**「縮退モード」** に切り替わる。
...
Vercel Cron `/api/cron/tenant-monthly-reset` が毎月 1 日 UTC 00:00（JST 09:00）に動作。`lastResetAt` が前月以前のテナントを検出して上記処理を実行する。**PR-4 以降はテナントの `timezone` に応じた月境界判定** に対応。
