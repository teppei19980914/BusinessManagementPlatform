...
**F-13.10 課金単位の統一**: 課金カウントの「1 回」は **ユーザに見える機能単位** で定義する。新規プロジェクト作成時の自動タグ抽出 + 初回提案生成は内部的に複数の LLM / Embedding 呼び出しを伴うが、ユーザから見える 1 操作として 1 回とカウントする。

**embedding 生成の課金扱い (PR #357 + #358 + #359 / 2026-05-14 改訂)**:
- **Project の新規/更新**: text (purpose/background/scope) 変更時に 1 回課金 (visibility 概念なし、常に対象)
- **Knowledge / RiskIssue / Retrospective の新規/更新**: `visibility='public'` (全メンバー公開) のときのみ 1 回課金。`visibility='draft'` (自分のみ) は **課金対象外** (= 提案エンジンの検索対象外なので embedding を生成する必要がない)
- **draft → public 遷移**: 公開化した瞬間に embedding 初回生成 = 1 回課金
- **public → draft 退行**: 既存 embedding は保持、追加課金なし
- **外部 import (CSV/XLSX)**: 取込全件を 1 ApiCallLog に集約 (= **N 件取り込み = 1 回課金**)。draft 行は集約からも除外され完全に課金対象外

これにより「ApiCallLog 件数 = `Tenant.currentMonthApiCallCount` = 画面表示の『今月 API 呼出』 = ユーザに見える実呼出回数」が統一される。
...
> **改訂 (PR #357 + #358 + #359 / 2026-05-14)**: 上記の旧設計は「embedding 完全非課金」を前提としていたが、現行実装では embedding 生成も `ApiCallLog` に記録され、テナント課金単価で計上される。一方で:
> - `visibility='draft'` (自分のみ) の資産は embedding を生成しない → 課金されない
> - 外部 import は **N 件 → 1 ApiCallLog** に集約 → 大量取り込みでも 1 回課金で完結
>
> 詳細マトリクスは §13.7.2 F-13.10 を参照。
