...
- §5.X+10 GitHub Actions のサプライチェーン汚染回避 (公式 install スクリプト化) / §5.X+11 `api.github.com` 共有 IP rate limit 回避 (releases/latest/download URL) / §5.X+12 「常に最新」CLI のメジャー breaking change 対策 (subcommand 明示・公式ソースで flag 確認)
