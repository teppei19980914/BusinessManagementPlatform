...
- HAR 計測: プロジェクト詳細画面の RSC レスポンスが **2.9 秒 / 149 KB**（他画面はほぼ 300ms 前後）
- Performance Trace 計測: レンダリングタブのボトルネック分析

(HAR / Trace の生データバイナリは PR #101 でリポジトリ容量整理のため削除。
必要になったら同手順 [docs/performance/20260417/performance-improvement-journey.md
§「計測手順」参照] で再取得可能。要約結果は本 md と
`docs/performance/20260417/after/次期プログラム/comparison-report.md` に転写済み。)
...
- 計測データ要約: `docs/performance/20260417/after/次期プログラム/` (comparison-report / bottleneck-and-fixes / cold-start-and-data-growth-analysis の 3 md) + `docs/performance/20260417/performance-improvement-journey.md` (生 HAR / Trace は PR #101 で削除、md に転写済)
