...
## 5.X+30 「ログインできない」の真因は別経路にあることが多い ─ 切り分け手順と防御的 server component (PR fix/admin-users-defensive-render / 2026-05-15)
<!-- 元番号 §5.X+28 から §5.X+30 に renumber (main 側 PR-4 = §5.X+28 と衝突回避。PR-N 予約番号体系は §5.X+25〜+29 で運用中) -->
...
## 5.X+31 「invalid_password と記録されているが本人は正しい入力 + パスワードマネージャ使用」のパターン ─ authorize() 例外で auth_event_logs に何も残らないケースが多い (PR fix/auth-diagnostics-defensive / 2026-05-15)
<!-- 元番号 §5.X+29 から §5.X+31 に renumber (main 側 PR-5 = §5.X+29 と衝突回避) -->
...
- 過去の関連 §5.X+30: 「ログイン直後 server component crash = ログインできない感」
