...
E2E テスト追加 (新規プロジェクト作成 → タグ自動抽出 → 提案表示)、視覚回帰テスト、本番環境での段階的有効化 (feature flag 経由)、ヘルプドキュメント連携。最終的なセキュリティ確認は `.github/workflows/security.yml` の CI 自動検査 (閾値 90/100、2026-05-09 以降のルール) で担保。所要見込み 4〜5 日 (うち 1〜2 日はバッファ)。
