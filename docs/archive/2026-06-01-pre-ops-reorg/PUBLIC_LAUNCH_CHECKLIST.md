...
- [x] **公式マスコット「たすきフクロウ」のロゴ配置**(2026-05-28 / feat/login-setup-guide-link、`src/app/(auth)/login/page.tsx` の CardHeader 左横に `public/mascot-owl.png` を 40×40 で配置。AppHeader と同パターン)。**2026-05-29 更新 (feat/login-mascot-and-layout-fix / KDD §5.X+177)**: PR #451 (mascot 導入) 以降、middleware matcher が `public/` 配下の静的ファイルを exclusion 列挙しておらず、`/mascot-owl.png` / `/mascot-owl-chat.png` / **`/og-image.png` (SNS シェアプレビュー)** / **`/robots.txt` (招待制 noindex)** が /login に 302 redirect されていた重大バグを発見・修正。middleware matcher に拡張子ベースの regex `[^?]+\.(?:png|jpg|jpeg|svg|webp|gif|ico|txt|xml|woff2?|ttf|eot)$` を追加して一括除外。Round 1 で「Optimizer 不安定」と誤診断し `unoptimized` を 5 箇所付与した false trail は Round 2 verification で撤回、payload 最適化を復活させた。回帰防止: `src/middleware.test.ts` + `e2e/specs/17-public-static-assets.spec.ts` を新設
