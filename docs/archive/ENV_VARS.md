# 環境変数一覧 (Operations) — ⚠️ ARCHIVED (2026-05-30)

> 🗄️ **本ドキュメントは archive されました (2026-05-30)**。環境変数の正は **[docs/design/ENVIRONMENT_VARIABLES.md](../design/ENVIRONMENT_VARIABLES.md)**(as-built インベントリ + 取得方法・運用注意・context マトリクスを §11〜§13 に統合)です。二重管理防止のため本書は更新しません(過去の経緯参照用に凍結)。
> ※ 本書内の相対リンクは旧 `docs/operations/` 基準のため一部リンク切れがあります。

---

本ドキュメントは、本サービスの全環境変数を一覧化する (OPERATION.md §1 を転記)。デプロイ手順は DEPLOYMENT.md、ローカル起動は SETUP_LOCAL.md を参照。
...
| `CRON_SECRET` | (任意のランダム文字列) | 外部 cron (cron-job.org) から `/api/admin/users/lock-inactive` 等を叩く際の `Authorization: Bearer` で使用。**未設定の場合 cron は実行されない** (手動実行は admin ログインで可能)。PR #89 で 30 日非アクティブユーザに使用 (feat/account-lock 改修で **論理削除 → ロック (isActive=false)** に方針変更)。 |
...
| `STRIPE_PRICE_EMBEDDING` | (未設定 / **optional**) | **ADR-0022 (2026-06-01) 新規 + ADR-0029 (2026-05-30) ¥1→¥5 改定**: Embedding 機能 (Meter event `tasukiba_embedding_call`) の Price ID。**¥5/call、Metered** (Expert/Pro 共通、Beginner は ¥0 維持 = LP「90 日完全無料」訴求保全)。**✅ 2026-05-30 Production 設定完了** (= 6/1 credit_card 払い有効化、PR #469 + Sandbox→Live mode 移行 TC-L1〜L8 PASS 後)。設定時は `createSubscriptionForTenant` が 5 本目の Subscription Item (Haiku/Sonnet/**Embedding**/DBCap/Storage) として追加し、`stripe-usage-flush` が embedding queue を送信する **5 Item invariant** 構成。Test / Live で別 Price ID。⚠️ **Production 値は必ず Live Account ID `KHIaXKbo0M` が含まれること** (Sandbox `K3TUQWW2eq` 混入で `No such price` 400 fail、TC-L4 検証時に検出した罠) |
...
| **`ANTHROPIC_API_KEY`** ★必須★ | Anthropic Console 発行の API key | **全 context (Production / Deploy preview / Branch) で必須**、★scope は Builds + Functions + Runtime★ (= deploy build 時にも参照可能にする)。auto-tag / suggestion-explanation / help-chat の全 LLM 経路で参照。未設定だと該当 endpoint が 503 を返す ─ ヘルプチャットは [route.ts root-level catch](../../src/app/api/help/chat/route.ts) で 503 + `fallbackToAccordion:true` に倒すが、提案エンジン系はテナント書込阻害になるため preview にも必ず設定する。詳細: [DEPLOYMENT.md §4.4](./DEPLOYMENT.md) |
| **`VOYAGE_API_KEY`** ★必須★ | Voyage AI ダッシュボード発行の API key | **全 context で必須**、★scope は Builds + Functions + Runtime★ (= Netlify build hook で `tsx scripts/generate-faq-embeddings.ts` が deploy 時に自動実行されるため build 時にも必要、KDD §5.X+196)。chat-semantic-search / 各 entity の embedding / help-chat の RAG embedding (ADR-0028) で参照。未設定だと意味検索が pg_trgm fallback に縮退、ヘルプチャットは degraded mode、build hook FAQ embedding 生成は WARN ログのみで skip (= build 自体は成功)。Voyage 200M tokens/月 無料枠は全 context 合算でも 1% 未満で吸収可能 |
