...
| 変数名 | 説明 | 例 |
|---|---|---|
| DATABASE_URL | PostgreSQL 接続文字列 | postgresql://user:pass@localhost:5432/bmp?connection_limit=10&connect_timeout=5 |
...
データ量の増加に応じて、以下のテーブルに月次パーティショニングを検討する。

| テーブル | パーティションキー | 導入目安 |
|---|---|---|
| audit_logs | created_at | 100万レコード超過時 |
| operation_trace_logs | created_at | 100万レコード超過時 |
| auth_event_logs | created_at | 100万レコード超過時 |
| task_progress_logs | created_at | 50万レコード超過時 |
...
| 項目 | 設定値 | 理由 |
|---|---|---|
| Prisma 接続プールサイズ | 10 | 同時 30 ユーザに対して十分 |
| 接続タイムアウト | 5 秒 | プール枯渇時の待機上限 |
| アイドルタイムアウト | 300 秒 | 不使用接続の解放 |

```
DATABASE_URL="postgresql://user:pass@host:5432/db?connection_limit=10&connect_timeout=5"
```
...
| ログ記録 | 送信成功/失敗を operation_trace_logs に記録 |
