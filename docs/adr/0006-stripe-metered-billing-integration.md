...
- **Default テナント (運営者自身)**: ADR 採択時は credit_card 払い未対応 (= 内部運用テナントのため対象外) としていたが、**2026-05-30 launch 検証 (TC-L1〜L8) で credit_card 経路の動作確認に使用済**。launch 後の運用ポリシー (= Default を invoice に戻すか credit_card で継続) は別途確定要
