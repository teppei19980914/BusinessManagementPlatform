...
| 段階 | 環境 | 対象 | 理由 |
|---|---|---|---|
| **① マージ判断 (主)** | **Deploy Preview / staging** (`deploy-preview-<PR番号>--tasukiba.netlify.app` 等) | **SMK-1〜7 全部** | **実外部サービス (Voyage/Anthropic/メール/Supabase Storage) + ステージング DB** に接続済のため、実連携を検証でき**本番 DB を汚さない** (払い出し→解約の破壊的ライフサイクルも安全)。本プロジェクトの人間 UAT 環境 ([DEPLOYMENT.md §Deploy Preview](../operations/develop/DEPLOYMENT.md)) |
