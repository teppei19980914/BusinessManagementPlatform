...
4/15        5/1          6/1          7/1
 |           |            |            |
 v           v            v            v
 開発開始    MVP-1a       MVP-1b       MVP-2
             リリース      リリース      リリース

 MVP-1a:                MVP-1b:              MVP-2:
 最小動作版              機能拡充版            セキュリティ強化版
 - 認証基盤             - 見積もり管理        - MFA（TOTP）
 - プロジェクト管理      - ガントチャート       - パスワードリセット
 - タスク管理           - リスク/課題管理      - アカウントロック
 - ナレッジ管理         - 振り返り            - 自動削除バッチ
 - ユーザ/メンバー管理  - マイタスク           - 監査ログ画面
 - 権限制御             - メール検証
 - 監査ログ(記録)       - CSV エクスポート
```
...
| フェーズ | リリース日 | 目的 | 利用開始条件 |
|---|---|---|---|
| MVP-1a | 2026-05-01 | 最小動作版。試験運用を開始 | プロジェクト管理 + タスク管理 + ナレッジ蓄積が動作 |
| MVP-1b | 2026-06-01 | 機能拡充。見積もり〜振り返りの全業務フローを完成 | 全ビジネス機能が動作 |
| MVP-2 | 2026-07-01 | セキュリティ強化。外部展開に耐えうる品質 | MFA・パスワードリセット等が動作 |
...
本計画書は MVP の開発スケジュールを定義するドラフトである。
試験運用のフィードバックや技術的制約に応じて改訂する。
