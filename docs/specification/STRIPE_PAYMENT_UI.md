...
**feat/tenant-settings-tabs (2026-05-22) 以降**: `/settings/tenant` 画面は 3 タブ構成 (概要 / 使用量 / 請求) に再編済。
本「支払い方法」セクションは **「請求」タブ内** に配置され、`/settings/tenant?tab=billing` で直接遷移可能。

タブ内では「請求先情報」セクション (= paymentMethod セレクトを含むフォーム) の **直下** に「支払い方法」セクションを配置し、
さらに末尾に「請求履歴」(= `/settings/tenant/billing` へのリンク) セクションが続く構成。
...
請求タブが選択された状態で着地する (= ユーザが操作開始した動線から離れない)。
