...
> **★ドリフト修正 (2026-06-19)★** 本節は以前「feat/gantt-tab-restructure (2026-04-26) でガントを WBS タブ内ボタンに統合した」と記載していたが、**2026-04-30 に独立タブへ復帰**しており実装と不一致だった。プロジェクト詳細には現在も「進捗確認」(旧称: ガントチャート、内部 tab key は後方互換のため `gantt` を維持) が **WBS 管理と並ぶ独立タブ** として存在する。

- **プロジェクト単位**: プロジェクト詳細 → 「進捗確認」タブ → 画面上部のチップで「ガント表示」「担当者別確認」(v1.3.0 追加) を個別に表示/非表示切替 (両方同時表示も可)
- **横断**: マイタスク画面 → 「ガントチャートを表示」ボタン → 自分の担当タスクをプロジェクトごとに縦並びで Gantt 表示 (担当者別確認ボードはプロジェクト単位のみで、マイタスク側は未対応)
