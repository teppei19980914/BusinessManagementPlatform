...
| `STRIPE_PRICE_EMBEDDING` | (未設定 / **optional**) | **ADR-0022 (2026-06-01) 新規**: Embedding 機能 (Meter event `tasukiba_embedding_call`) の Price ID。**¥1/call、Metered** (Expert/Pro 共通、Beginner は ¥0 維持 = LP「90 日完全無料」訴求保全)。**リリース時は未設定** (= 2026-06-01 リリースで credit_card 払い未対応、invoice/bank_transfer 経由で課金完結)。将来 Stripe Dashboard で Meter + Price を作成して本 env を設定すると `createSubscriptionForTenant` が 3 本目の Subscription Item として追加し、`stripe-usage-flush` が embedding queue を送信開始する **Stripe-ready 設計**。Test / Live で別 |
