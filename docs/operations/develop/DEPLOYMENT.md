...
障害対応は [INCIDENT_RESPONSE.md](../operate/INCIDENT_RESPONSE.md)、ロールバックは本書 §7 ロールバック手順を参照。
...
> ⚠️ **本番切替前のチェックリスト**: Stripe Dashboard 側で Live mode の Product / Price / Webhook endpoint がすべて作成済かを確認。詳細は [`STRIPE_SETUP.md`](../setup/STRIPE_SETUP.md) と [`docs/business/STRIPE_BILLING.md`](../../business/STRIPE_BILLING.md)。
...
cron-job.org に登録する cron は **計 13 件** = [`src/config/cron-jobs.ts`](../../../src/config/cron-jobs.ts) の `CRON_JOBS` **12 件** + `/api/health` (health-check、`CRON_JOBS` には未掲載) **1 件**。真値は常に `src/config/cron-jobs.ts` とする (下表は代表例の抜粋であり、最新の全件・スケジュールは同ファイルを参照)。旧 `vercel.json` から移行した cron schedule + ADR-0021 で追加:
...
| **`/api/cron/attachment-embedding`** | **`*/10 * * * *` (10 分毎)** | **ADR-0021 (2026-05-26): 添付ファイル本文 embedding 背景処理 (pending → completed、指数 backoff 3 回 retry、per-tenant=5 / global=50 throttle)** |

### 6.2 cron-job.org 設定手順

1. <https://cron-job.org/en/signup/> でアカウント作成 (無料、cron 数無制限)
2. 「Create cronjob」を 13 件作成、各々以下を設定:
...
DB スキーマ変更を伴うロールバックは 本書 §7 (ロールバック手順) を参照。
...
- 障害対応手順は [INCIDENT_RESPONSE.md §6.11](../operate/INCIDENT_RESPONSE.md) を参照。
