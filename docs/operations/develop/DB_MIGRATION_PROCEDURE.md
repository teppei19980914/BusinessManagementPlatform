...
| Netlify 環境変数 `DATABASE_URL` | `postgresql://postgres.[ref]:[pw]@aws-1-ap-northeast-1.pooler.supabase.com:6543/postgres?pgbouncer=true` (Transaction pooler) | Netlify |
| Netlify 環境変数 `DIRECT_URL` | `postgresql://postgres.[ref]:[pw]@aws-1-ap-northeast-1.pooler.supabase.com:5432/postgres` (Session pooler、port **5432** 注意) | Netlify |
...
> **⚠ この一覧は陳腐化前提**: 下表は #18 (`20260427_stakeholders` / 2026-04-27) で更新が止まっており、それ以降に追加された migration (例: `20260525_tasks_unique_parent_name` 等) は反映されていない。**実際に本番へ適用済みかどうかの真値は常に `_prisma_migrations` テーブルとする** (= 下表は履歴の参考であり、適用状況の判断には使わない)。確認は Supabase SQL Editor で `SELECT migration_name, finished_at FROM _prisma_migrations ORDER BY finished_at DESC;` を実行し、`prisma/migrations/` ディレクトリと突き合わせる。
