...
> **注 (当時)**: 下表は PR #77 / #81 時点 (2026-04 初頭) のスナップショットであり、その後の i18n 一括移行 (worktree agents による大規模翻訳等) は反映していない。最新の移行状況は `src/i18n/messages/ja.json` と `src/i18n/messages.test.ts` の `REQUIRED_*_KEYS` を真値とする。
