...
詳細は [docs/operations/develop/DB_MIGRATION_PROCEDURE.md](./DB_MIGRATION_PROCEDURE.md) §3 (DB マイグレーション手順) を参照。
...
日常の機能 PR をまとめて新バージョン (SemVer の MAJOR / MINOR / PATCH) として **告知付きで公開する** 場合は、上記の手順に加えて以下を必ず実施 (= 真値ファイルの更新漏れがあると `/changelog` / `/announcements` で旧情報が表示され続ける):
...
bug fix 1 件だけの hot fix では 1 / 2 のみ、メジャーリリース時は更に [PUBLIC_LAUNCH_CHECKLIST.md](../../archive/2026-06-01-pre-ops-reorg/PUBLIC_LAUNCH_CHECKLIST.md) も実施する等、リリース規模で必要手順が異なるため必ず元ドキュメントで判断する。
