...
>
> 日常の機能 PR (= 個別の `feat:` / `fix:`) を main にマージするだけのフローは [docs/developer-guide/COMMIT_AND_DEPLOY.md](../developer-guide/COMMIT_AND_DEPLOY.md) を参照。本書は「ユーザに告知して公開する単位 = リリース」を対象とする。

---

## 1. リリース時に更新する真値ファイル一覧

公開ページ (`/changelog` / `/announcements` / `/settings/about`) は本ファイル群を真値として描画される。**リリースのたびに以下を必ず確認**すること。

| # | 真値ファイル / 設定値 | 反映先 (公開 UI) | 説明 |
|---|---|---|---|
| 1 | [`CHANGELOG.md`](../../CHANGELOG.md) (リポジトリ root) | `/changelog` 公開ページ + フッタ「サービス情報」 経由 | Keep a Changelog 1.1.0 形式。SemVer の各バージョンに「追加 / 変更 / 修正 / 削除」を記載 |
| 2 | [`docs/public/announcements/{YYYY-MM-DD}-{slug}.md`](../public/announcements/) | `/announcements` 一覧 + `/announcements/{slug}` 詳細 | 個別お知らせの markdown ファイル。frontmatter (`title`, `publishedAt`, `severity`, `summary`) + 本文 |
| 3 | [`package.json`](../../package.json) の `version` | フッタ表示 (3 段目で間接参照) / `/settings/about` の「現在のバージョン」 | SemVer (`MAJOR.MINOR.PATCH`) を bump |
| 4 | `NEXT_PUBLIC_RELEASE_DATE` 環境変数 (Netlify Site settings) | `/settings/about` の「リリース日」 / フッタ「最終更新」 | ISO-8601 (`YYYY-MM-DD`)。未設定時の fallback は [`src/lib/app-version.ts`](../../src/lib/app-version.ts) の `FALLBACK_RELEASE_DATE` |
| 5 | [`src/config/operator.ts`](../../src/config/operator.ts) | フッタ copyright / `/settings/about` の「運営者」 | 運営者氏名 / 表示ラベル / 連絡先 URL。改姓・改名・問い合わせ口変更時のみ |
| 6 | [`src/config/legal-versions.ts`](../../src/config/legal-versions.ts) | `/settings/about` の「利用規約」「プライバシーポリシー」 | LP 側 anchor URL。法的書面の改定時に更新 |

> **削減済の経路 (2026-05-24 改訂)**: 旧フッタには「サービス名 / 運営者 / 更新履歴 / 利用規約 / プライバシー / お問い合わせ」が直接並んでいたが、UX 向上のため `/settings/about` に集約済。リリース時に「フッタ表示文言」を個別に編集する必要は無い。
...
- [ ] **(7) Netlify Production deploy 成功確認** ([COMMIT_AND_DEPLOY.md §10.5 squash merge 時の skip キーワード罠](../developer-guide/COMMIT_AND_DEPLOY.md))
- [ ] **(8) `/settings/about` を本番で開き、バージョン番号とリリース日が反映されていることを確認**
- [ ] **(9) `/changelog` `/announcements` を本番で開き、新エントリが表示されることを確認**

### 2.2 メジャーリリース (例: v0.x → v1.0.0)

上記 2.1 に加えて:

- [ ] [`docs/operations/PUBLIC_LAUNCH_CHECKLIST.md`](./PUBLIC_LAUNCH_CHECKLIST.md) の全項目を完了
- [ ] [`docs/operations/GO_LIVE_RUNBOOK.md`](./GO_LIVE_RUNBOOK.md) の T-2 週間前 / T-1 週間前 / 当日 のタイムラインを実施
- [ ] OG 画像 (`public/og-image.png`) の差し替え判断
- [ ] 利用規約 / プライバシーポリシー (LP 側) の改定確認
- [ ] [`docs/operations/RELEASE_NOTES_v1.md`](./RELEASE_NOTES_v1.md) 相当のリリースノート (PDF 配布等が必要な場合のみ)
...
**critical 告知が必要なインシデント時**は [`docs/operations/INCIDENT_RESPONSE.md`](./INCIDENT_RESPONSE.md) も合わせて参照。
...
- [docs/developer-guide/COMMIT_AND_DEPLOY.md](../developer-guide/COMMIT_AND_DEPLOY.md) — 日常コミット・デプロイのワークフロー
- [docs/operations/PUBLIC_LAUNCH_CHECKLIST.md](./PUBLIC_LAUNCH_CHECKLIST.md) — 一般公開前の包括チェックリスト
- [docs/operations/GO_LIVE_RUNBOOK.md](./GO_LIVE_RUNBOOK.md) — 2026-06-01 GA リリースの当日進行
- [docs/operations/DEPLOYMENT.md](./DEPLOYMENT.md) — Netlify deploy / skip キーワード / ロールバック手順
- [docs/operations/INCIDENT_RESPONSE.md](./INCIDENT_RESPONSE.md) — critical 告知時の社内手順
