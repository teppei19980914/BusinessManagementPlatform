...
#### 緊急停止フラグ `SUGGESTION_ENGINE_DISABLED` (全テナント一括の最終手段)

提案エンジン (Anthropic タグ抽出 + Voyage embedding + 「なぜ?」説明文) を **全テナント一括で即時停止**する環境変数。LLM/embedding 呼び出しが完全にゼロになり、提案機能は空配列を返す (fail-safe)。

**停止手順** (5 分以内):

1. Netlify Dashboard → Site configuration → Environment variables
2. **`SUGGESTION_ENGINE_DISABLED`** = **`true`** を追加 (Production スコープ)
3. Netlify Dashboard → Deploys → 最新を **Trigger deploy → Deploy site** (環境変数を反映するため再デプロイが必要)
4. デプロイ完了後 (~2 分)、提案機能が空配列を返すようになり LLM / embedding 呼び出しもゼロに

**復旧手順**: `SUGGESTION_ENGINE_DISABLED` を **削除** (または `false`) → 再デプロイ。

**シナリオ別の使い分け**:

| シナリオ | 症状 | 対処 |
|---|---|---|
| **A. LLM API 障害で大量エラー** | Netlify Function ログに `voyage_api_error` / `anthropic_api_error` が大量、プロジェクト作成が遅い | `SUGGESTION_ENGINE_DISABLED=true` で全停止 → プロバイダ復旧後に解除 |
| **B. 月次予算超過で課金を即停止** | 当月使用量が想定を大幅超過、課金を止めたい | 同上。タグ抽出・embedding 生成 (= 課金対象呼び出し) が全停止し課金もゼロに |
| **C. 特定テナントのみ抑制** | 1 テナントだけ異常利用 (他テナントは正常稼働させたい) | フラグは全テナント一括のため使わない。代わりに該当テナントの `monthly_budget_cap_jpy` を 0 等に設定し `withMeteredLLM` 側で `reason: 'budget_exceeded'` 拒否させる (下記 SQL) |
...
これで該当テナントの LLM 呼び出しのみが拒否され、他テナントは通常通り稼働する。

#### トラブルシューティング: 提案が表示されない (空)

以下を順に確認する:

1. **`SUGGESTION_ENGINE_DISABLED=true`** が設定されていないか (Netlify 環境変数を確認)
2. default-tenant に **シードナレッジが投入済**か (`SELECT COUNT(*) FROM knowledges WHERE tenant_id='00000000-0000-0000-0000-000000000001' AND visibility='public';`)
...
#### トラブルシューティング: embedding が NULL のまま効かない

新規データには自動生成されるが、既存データには **月初バッチ** (`runMonthlyEmbeddingBackfill` / 外部 cron で実行) で補完される。手動再実行が必要な場合は §6.8 の月初 cron 手動再実行手順を参照。なお embedding が NULL でも **重み再配分の縮退モード (タグ:テキスト = 5:5)** で動作するため致命的ではない。

#### トラブルシューティング: Anthropic コンソールのクレジット残高が減らない

仕様通り。本サービスの内部請求 (`Tenant.currentMonthApiCostJpy`、プラン別固定単価 Beginner ¥0 / **Expert ¥10/call** / **Pro ¥15/call**) と Anthropic の実 API 課金 (実トークン消費) は **完全に独立した 2 系統**。「クレジットが減らない」と感じたら以下を確認:

1. **API key 設定**: Netlify 環境変数 `ANTHROPIC_API_KEY` が正しいか / 該当 key の Console を見ているか
2. **呼び出し発火**: `api_call_logs` に該当 `feature_unit` (`project-upsert` / `suggestion-explanation` 等) のレコードが入っているか
3. **集計遅延**: Anthropic Console は数十秒〜数分の遅延があるため即時反映を期待しない
4. **無料クレジット**: 新規アカウントは無料クレジット枠から先に消費されるため有償残高が動かないように見えることがある

呼び出し有無の確認 SQL (現行 featureUnit 一覧):
...
### 6.13 ストレージ容量逼迫 / 計測失敗 (S-3、2026-05-31 ADR-0030 改定)

DB 容量 / ファイルストレージの使用量が大きいテナントの検知、または storage-guard の計測失敗が出た場合。

> **前提 (ADR-0030「データはたすきばの命」)**: **累積容量では write は止まりません**。L1 (1GB) / L2 (10GB) / L3 (50GB) はいずれも super_admin への**監視アラート閾値**であり、書込を拒否しません。write が拒否されるのは次の 2 つだけです:
> - **Beginner 無料枠超過** (DB 50MB / Storage 100MB、Beginner プランのみ。DELETE は許可)
> - **1 操作あたりの上限超過** (DB write 5MB / ファイル 50MB/件、全プラン共通)
>
> 旧来の「累積 50GB ハードキャップによる write 拒否」と「計測失敗時に write を止める circuit breaker (fail-close)」は ADR-0030 で**いずれも撤廃**されました。

#### 症状

- 運営ダッシュボードの DB/ファイルストレージ従量課金 alert カードで特定テナントが L2 (10GB) / L3 (50GB) に到達
- Netlify Function ログに `[storage-guard] DB usage 計測失敗 (fail-open, tenant=...)` (`kind: 'storage_guard_measure_failed'`)
- Supabase Compute の cache hit ratio 低下 / クエリ遅延 (noisy-neighbor 兆候)

#### 対処

| 状況 | 対処 |
|---|---|
| 特定テナントが L3 (50GB) 到達 | **write は止まっていない** (= 緊急対応不要)。Supabase Compute サイズ (Micro 等) と instance-wide 使用量 (L4 alert) を確認し、必要なら **Compute 増強** を検討。誤請求懸念があれば `POST /api/admin/super/tenants/[id]/recalculate` で再集計し peak/課金根拠を確認 |
| storage-guard の計測失敗ログ (fail-open) | **write は止まっていない** (記録のみ skip)。原因 (動的 SQL 失敗 / DB 高負荷 / schema 変更) を調査。真値は日次 cron `updateAllStorageBytesUsed` が再計測して補正するため、課金 (月内 MAX) は取りこぼさない。恒久的に失敗が続く場合のみ手動 recalc を実行 |
| instance-wide で cache hit ratio 劣化 (noisy-neighbor) | 累積で write を止める運用は行わない。Supabase Compute 増強で吸収する (将来、super_admin 画面に cache hit ratio 表示を追加予定 — [ROADMAP.md](../ROADMAP.md) C 参照) |

> **circuit breaker / storage-guard-reset は dormant**: 旧 circuit breaker は撤去済のため「circuit open による全 write 拒否」「`storage-guard-reset` での復旧」は**発生しません**。`storage-guard-reset` route は残置されていますが未使用で別 PR 撤去予定 ([ADMIN_DASHBOARD.md](./ADMIN_DASHBOARD.md) 主要運用操作まとめ参照)。
