...
### 33.3 CSV フォーマット (新形式、17 列)

| # | 列名 | 必須 | 区分 | import 動作 |
|---|---|:---:|---|---|
| 1 | ID | △ | 突合 | 値あり=UPDATE、空欄=新規作成 |
| 2 | レベル | ○ | 階層 | 親子関係再構築 (1〜N) |
| 3 | 種別 | ○ | 階層 | WP / ACT。切替不可 (dry-run でエラー) |
| 4 | 名称 | ○ | 計画 | UPDATE 反映 |
| 5 | WBS番号 | △ | 計画 | UPDATE 反映 |
| 6 | 担当者氏名 | △ | 計画 | ProjectMember 一意 lookup、複数該当=エラー |
| 7 | 予定開始日 | △ | 計画 | UPDATE 反映 (ACT のみ、WP は子から自動算出) |
| 8 | 予定終了日 | △ | 計画 | 同上 |
| 9 | 見積工数 | △ | 計画 | 同上 |
| 10 | 優先度 | △ | 計画 | `low` / `medium` / `high` |
| 11 | マイルストーン | △ | 計画 | `○` / 空 |
| 12 | 備考 | △ | 計画 | UPDATE 反映 |
| 13 | ステータス ※read-only | × | 進捗 | 無視 (DB と異なれば dry-run で warn) |
| 14 | 進捗率 ※read-only | × | 進捗 | 同上 |
| 15 | 実績工数 ※read-only | × | 進捗 | 同上 |
| 16 | 実績開始日 ※read-only | × | 進捗 | 同上 |
| 17 | 実績終了日 ※read-only | × | 進捗 | 同上 |

ヘッダー行に `※read-only (import 時無視)` を 13〜17 列に明示。BOM 付き UTF-8 (Excel 対応)。

### 33.4 API 仕様

#### 33.4.1 エクスポート (既存ルート拡張)

`POST /api/projects/[id]/tasks/export`

旧テンプレート互換性のため、`mode` パラメータで分岐する:

```typescript
// Body
{
  taskIds?: string[];        // 既存: 選択タスクのみ。未指定なら全件
  mode?: 'template' | 'sync'; // 新規。既定 'template' (互換)
}
```

- `mode='template'` (既定): 旧 10 列フォーマット、担当者リセット、進捗リセット
- `mode='sync'`: 新 17 列フォーマット、担当者・進捗 DB 値そのまま、ID 列含む
