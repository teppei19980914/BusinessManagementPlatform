...
`User` テーブルに `subscription_tier`、`current_month_token_usage`、`monthly_token_limit`、`last_token_reset_at` のカラムを追加し、月間使用量を増減・確認する純関数を整備する。Vercel Cron による月初リセットジョブを実装する。Upstash Redis を Vercel ダッシュボードから有効化し、`@upstash/ratelimit` パッケージを導入する。LLM 呼び出しを行う将来の API ルートで使用する共通ミドルウェアの雛形を準備する。所要見込み 3〜4 日。
...
Vercel Cron もしくは初回 deploy 時に動作する `prisma/seed-suggestion.ts` を新設し、ユーザが選定・要約した 30〜100 件のナレッジを「システム所有のプロジェクト (system-seed-project)」配下に登録する。これらは visibility=public で全ユーザに対する提案候補として機能する。所要見込み 2〜3 日 (実装はユーザの要約作業と並行)。
...
5月10日 (Week 1 末) には PR #2 (経済的安全性基盤) と PR #3 (Phase 1) が完成している。5月17日 (Week 2 末) には PR #4 と PR #5 (Phase 2 基盤と統合) が完成している。5月24日 (Week 3 末) には PR #6 と PR #7 (初期データと監視) が完成している。5月31日までに PR #8 (統合テスト) を完了し、6月1日リリースに臨む。

ただし、5月25日時点で進捗が 80% に達していない場合は、リリース日を 6月8日に 1 週間延伸する判断を予め決めておく。これは「品質を担保せずにリリースして悪用される方が、1 週間の延伸より遥かに重大なダメージ」という経営判断の表明である。
