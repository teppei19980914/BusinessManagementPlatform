...
| T-22 | **【重要 / Phase 22a/b/c/d 分割】** 「○○一覧」へのインポート/エクスポート機能の本格実装 (項目 1) | ユーザ要望: 各「○○一覧」(risks/issues/retros/knowledge/memos) でデータの **エクスポート (登録済みデータの外だし) と インポート (新規作成 + 既存更新、削除なし)** をサポート。**現状**: risks のみ PMO 報告用 export 既存 (`/api/projects/[id]/risks/export` GET、8 列サマリ format)、tasks (WBS) は PR-ζ + T-19 完了で 7 列 sync-import あり。残 entity (retros/knowledge/memos + risks の往復編集用 sync-import) は未実装。**設計原則**: (1)「○○一覧」のみ対象、「全○○」は対象外、(2) 削除禁止 — ID 未指定=新規、ID あり=更新のみ、(3) WBS の sync-import パターン (dry-run + preview + apply の 3 段階) を踏襲、(4) §5.26 「共通部品流用」に従い `<EntitySyncImportDialog>` を共通化、(5) CSV format は編集 dialog の入力項目を完全網羅。**Phase 22a (先行実装、~1,100 行規模 — 1 PR)**: risks の sync-import 単体実装 + 共通基盤の確立。具体: (i) `src/services/risk-sync-import.service.ts` 新設 (parseSyncImportCsv / computeSyncDiff / applySyncImport、`task-sync-import.service.ts` の flat 版で parent stack 削除)、(ii) `src/app/api/projects/[projectId]/risks/sync-import/route.ts` 新設 (`?dryRun=1` + 本実行)、(iii) `src/components/dialogs/risk-sync-import-dialog.tsx` 新設 (`wbs-sync-import-dialog.tsx` を risks 用にコピー + 列定義置換)、(iv) `risks-client.tsx` に「インポート」ボタン追加 + handleSyncImport、(v) export 改修: 既存 PMO 用 8 列 export とは別に sync 用 16 列 CSV 出力 endpoint を新設 (or query param `?mode=sync` で切替)、(vi) test (parser + computeSyncDiff の単体テスト 6-8 件)、(vii) docs 更新。**Phase 22b/c/d**: Phase 22a で確立した dialog component pattern を retrospectives / knowledge / memos に **機械的横展開** (各 ~300 行)。**Risk CSV 列構成 (16 列、編集 dialog 完全網羅、§5.31 のアクション充足チェック適用済)**: ID / type / title / content / cause / impact / likelihood / responsePolicy / responseDetail / assigneeName (氏名 → userId lookup) / deadline / state / result / lessonLearned / visibility / riskNature。priority は service 層 `computePriority()` で自動算出のため CSV 非対応。**着手準備状況**: 設計完全確定済 (本日 §5.31「枠数固定要件のアクション充足チェック」適用済 / Risk CSV 列構成も §5.31 マトリクスでの CRUD 充足検証済)、次セッションで Phase 22a 着手可能 | 2026-04-28 ユーザ要望「インポート/エクスポート機能を追加」、PR-ε で設計のみ実施。本日 §5.31 で「枠数固定要件の事前検証」確立により T-22 着手前の設計強化が完了。Phase 22a の規模 (~1,100 行) は 1 PR に収まるが本セッションのキャパシティ (T-19/T-21/T-17/§5.31 で 4 commits 完了済) を超えるため次 PR に送る |
