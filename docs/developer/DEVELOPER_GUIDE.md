...
| 2026-04-28 | §10.5 新パターン「内容重複型」発見 + 予防ルール (PR #182 conflict resolve / Stop hook 補正)。これまで §10.5 9 例目「機械並列型」(独立 PR の末尾追記衝突) のみだったが、PR #182 の rebase で **新パターン「内容重複型」** を発見: 同じ調査結果を **別 branch / 別番号で 2 回記録** した結果、merge 時に「内容は同じだが番号と章タイトルが違う」コンフリクトが発生。PR-η の §5.28 (永続ロック調査・初版) と PR-β hotfix の §5.29 (永続ロック調査・新版、より情報量多) が同じ内容で重複し、HEAD ブロックを丸ごと削除して main の新版を採用する解消が必要だった。**予防ルール**: 重大バグ調査結果を docs に記録するときは、機能 PR ではなく **専用 docs PR** で記録する (重複リスク回避)、または `git log --all --grep='<keyword>'` で他 branch の同名記録を事前確認する。本予防ルールは将来の §10.5 改訂で正式統合予定 |
