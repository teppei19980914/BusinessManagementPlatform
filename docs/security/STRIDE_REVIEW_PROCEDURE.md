...
| 実施回 | 予定日 | スコープ | 備考 |
|---|---|---|---|
| **初回** | 2026-07-06 (月) 〜 07-12 (日) | **6/1 リリース後の全機能** に対する初回 STRIDE 分析 | 過去の SUGGESTION_ENGINE_THREAT_MODEL.md / PHASE2_THREAT_MODEL.md は機能単体の分析だったため、サービス全体としては初回 |
| 第 2 回 | 2026-10-06 (月) 〜 10-12 (日) | 7-9 月の機能追加 + 既存脅威モデルの更新 | |
| 第 3 回 | 2027-01-05 (月) 〜 01-11 (日) | 10-12 月の機能追加 + 年初の総括 | |
| 第 4 回 | 2027-04-06 (月) 〜 04-12 (日) | 1-3 月の機能追加 | |

#### 初回 (2026-07 初週) の重点項目

6/1 リリース直後のため、以下を特に重点的に分析:

- **テナント越境攻撃面の総点検**: ADR-0005 (二段階認可) が全 Service 関数に徹底されているか
- **LLM コスト爆発の現実的攻撃シナリオ**: per-API-call 課金の悪用パターンを想定攻撃者目線で再評価
- **Stripe 連携の脅威モデル** (v1.x の準備): Webhook 署名検証 / 重複イベント処理
- **Supabase Free プランの可用性リスク**: 1 週間自動 pause の影響範囲
- **MFA 暗号化キーの分離計画** (SECURITY-TASKS F-01) の進捗確認
