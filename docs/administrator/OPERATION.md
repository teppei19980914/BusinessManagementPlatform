...
| 未使用アカウント削除 | Vercel Cron | `POST /api/admin/users/cleanup-inactive` | 日次 03:00 UTC | `Authorization: Bearer ${CRON_SECRET}` または admin セッション |

※ `/api/cron/cleanup-accounts` は PR #115 で削除 (デッドコード)。`/api/admin/users/cleanup-inactive` に一本化した。vercel.json の `crons` も同 endpoint を参照。
