...
- [ ] **Phase 2**: pgvector + Voyage AI Embedding による意味検索
- [ ] **初期データ**: 資格試験事例・著名な法則を独自要約したナレッジ 30〜100 件
- [ ] **subscription_tier 基盤**: User テーブル拡張、月間トークン上限管理、Vercel Cron での月初リセット
- [ ] **5 層悪用防止**: シークレット保護 / 認証強化 / ユーザ単位 rate limit + トークン上限 / プロンプトインジェクション対策 / workspace 上限
- [ ] **監視・異常検知 (最小実装)**: llm_call_log + token_usage_audit + 日次集計 + admin 通知
...
- [ ] **観測ダッシュボード UI**: `/admin/observability/llm` (Phase 3c の一部)
