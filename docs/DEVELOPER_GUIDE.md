...
| `strict mode violation: getByText(...) resolved to 2 elements` | 同一テキストを含む要素が hydration 過渡や状態バッジ近傍で 2 つ以上一致する。片方が `visibility:hidden` でも strict mode は fail する | ✅ locator スコープを具体化する。`page.locator('h2').filter({ hasText: X }).first()` 等、要素種別まで絞る。`page.waitForLoadState('networkidle')` で過渡状態の待機も追加 (PR #93 hotfix 1) |
