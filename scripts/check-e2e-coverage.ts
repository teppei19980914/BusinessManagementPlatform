 ...
 *   docs/test/E2E_COVERAGE.md にエントリが欠けていれば CI で fail させる。
 ...
 *
 * 出力例:
 *   ❌ 以下の新規ファイルが docs/test/E2E_COVERAGE.md に未記載です:
...
function extractDocumentedPaths(): Set<string> {
  const docPath = path.join(ROOT, 'docs', 'test', 'E2E_COVERAGE.md');
...
function main() {
  ...
  console.error('❌ docs/test/E2E_COVERAGE.md に未記載の機能があります:');
  for (const m of missing.sort()) {
    console.error(`   - ${m}`);
  }
  console.error('');
  console.error('対応方法:');
  console.error('   docs/test/E2E_COVERAGE.md の該当セクションにエントリを追加してください。');
