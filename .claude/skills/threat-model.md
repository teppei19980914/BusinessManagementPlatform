...
**新機能を実装する場合は A を必ず先行実行**。Mode B はユーザ依頼時 / リリース前に実行する。

> **2026-05-09 改訂**: 旧仕様では「全 PR で Mode B-1 必須」だったが、開発効率向上のため
> **GitHub Actions `.github/workflows/security.yml` での自動スコア検査 (閾値 90/100) に一本化**。
> ローカル / Claude Code 側での Mode B-1 必須実行は廃止。ユーザが明示的に依頼した場合 (例:
> 大規模リファクタ後の追加検査) や、CI で score 低下が報告された場合に Mode B-1 を実行する運用へ。
...
- **CI 自動 (PR 単位)**: `.github/workflows/security.yml` で `pnpm tsx scripts/security-check.ts --min-score=90` が自動実行 (2026-05-09 以降このパスが正)
- **ユーザ依頼時**: 大規模リファクタ後の追加検査 / CI で score 低下が報告された時
- **リリース直前**: 全 Finding をレビューしスコア確認
- **(撤廃) PR 起票前 必須**: 2026-05-09 改訂で CI 自動に一本化、ローカル必須実行は廃止
...
## Mode B-1: PR 作成ワークフロー (ユーザ依頼時のみ実行)

> **2026-05-09 改訂**: 旧「全 PR で必須」を撤廃し、`.github/workflows/security.yml` の CI 自動実行に一本化した。
> 本セクションは **ユーザが明示的に依頼した場合 (CI で score 低下が報告された等)** の手動手順として残置。
...
### Mode B-1 のチェックリスト (ユーザ依頼時に確認 / 旧「PR 起票前 必須」は 2026-05-09 撤廃)
